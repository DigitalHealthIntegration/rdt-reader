...
class ObjectDetection(object):
    ...
    def __build_network__imgClass(self):   
            ...
            """
            inputs = tf.keras.layers.Input(shape=[20, 60, 1])
            ...
            #                             prediction_layer
            #                             ])
            conv1 = keras.layers.Conv2D(16,(5,5), padding='same',activation='relu',kernel_initializer=keras.initializers.lecun_uniform(seed=None))(inputs)
            conv2 = keras.layers.Conv2D(32,(3,3), padding='same',activation='relu',kernel_initializer=keras.initializers.lecun_uniform(seed=None))(conv1)
            maxpool1 = keras.layers.MaxPooling2D((2,2))(conv2)

            conv3 = keras.layers.Conv2D(64,(3,3), padding='same',activation='relu',kernel_initializer=keras.initializers.lecun_uniform(seed=None))(maxpool1)
            conv4 = keras.layers.Conv2D(128,(3,3), padding='same',activation='relu',kernel_initializer=keras.initializers.lecun_uniform(seed=None))(conv3)
            global_average_layer = tf.keras.layers.GlobalAveragePooling2D()
            feature_batch_average = global_average_layer(conv4)
            print(feature_batch_average.shape)
            prediction_layer = keras.layers.Dense(3,activation="softmax")
            prediction_batch = prediction_layer(feature_batch_average)
            model = keras.Model(inputs=inputs, outputs=[prediction_batch])
